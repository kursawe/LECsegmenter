...
#skimage.morphology has several modules that can alter morphology of images-skeletonize-binary to one-pic
from skimage.morphology import skeletonize
# this function is used to segment subsequent frames based on centroids from the previous frame
...
#the global keyword ensures the data can be updated 
...
#if you choose to start over we assign the value true
if answer=="Start Over":
    answer=True
    #if you choose to continue, we assign the value false. 
...
#merges_red overlaps the skeletonized image 2 (with red skeleton around membranes) with background which is image 1
...
#after binarizing the image (later),the membrane is assigned 0 and the background is assigned 1
#then find connected pixels and remove small connected objects in the image if under minimum size defined 
def removesmallelements(img,minsize):
    #to ensure the output img is the same size as input img
...
# and removes the incomplete cells on the edges of the image 
    ...
    for i in range(0, nb_components):
        if sizes[i] <=10 :
            output[output==i+1]=0
      #dilate segmentation boundaries-to make membrane thicker e.g if we have one white pixel on the membrane some neighboring white pixels surrounding the membrane will turn white
...
def get_stats(input_data):
    ...
    #dim1 and dim2 represent the x and y coordinates of the image 512x512 pixels sq
    ...
    MAX_NUMBER_OF_CELLS=max(nb_components)
...
#The output of this is the second image in the GUI sequence of images being displayed. 
 ...
 #Next, the skeletonized image is converted to a watershed and this is image 6 in the sequence of images. 
...
#update numbers is used to update the cell numbers each time you draw on the image in drawing mode
def update_numbers(membrane_outlines,frame_num,speed="Fast"):
    global Numbers, tiff_images, mask
    ...
    if frame_num < 1:
        ...
        mask=watershed2(membrane_outlines)[1]
        # this gets centroids from the current 
        centroids=get_centroids(mask)
...
#Channel 1 is the one channel image with red membranes; Numbers_mask is the image with text numbers in blue for each cell.  
...
#This is applicable to a video with several frames. The changes are saved into outcome. 
        ...
        if frame_num>0:
            Channel3_Mask,Channel1_Mask=watershed2(outline)
            outcome[frame_num][1]=Channel1_Mask.copy()
            outcome[frame_num][0]=Channel3_Mask.copy()
            Numbers_Mask=update_numbers(outline,frame_num,"Slow")
            outcome[frame_num][3]=Numbers_Mask.copy()
...
# get a list of centroid coordinates from previously segmented frame to use as seeds for watershed
def get_seed_coords(frame):
    Coord_list=[]
    nb_components=list(np.unique(frame))
    for cell_index in nb_components:
        #if the cell index is >1 i.e. since membrane=0; background=1 so >1 would be given to pixels inside the cells
        if cell_index>1:
            #np.where gives the mean coordinates of centroid; size>0 is likely a debugging step.
            if np.where(frame==cell_index)[0].size>0:
                #np.where(frame==cell_index)[0] denotes x coordinate, np.where(frame==cell_index)[1] denotes y coordinate
                Coord_list.append([int(np.mean(np.where(frame==cell_index)[0])),int(np.mean(np.where(frame==cell_index)[1]))])
#this will have x,y of centroid in each cell
    return (Coord_list)
    
# create a marker array with seeds labeled
def get_markers(frame):
    global num_seeds
    # create empty array of same size as the image to segment
    markers=np.zeros((dim1,dim2)).astype(np.uint8)
    # change values at coordinate of a centroid to 1
    seeds = get_seed_coords(frame)
    for seed in seeds:
            markers[seed[0],seed[1]] = 1
    #label each centroid (seed for watershed) with a different number
    markers, num_seeds = ndimage.label(markers)
...
# and removes the incomplete cells on the edges of the image 
    ...
    # this code uses previous centroids for a segmentation to label each cell
    output = segmentation.watershed(img, markers)
    nb_components = num_seeds - 1
    #stats = 
    ...
    sizes = np.bincount(output.flat)[1:]
    #for every component in the image, you keep it only if it's above min_size of 10pixels. otherwise give an output of 0.
    for i in range(0, nb_components):
        if sizes[i] <=10 :
            output[output==i+1]=0
    ...
    for i in range(0, nb_components):
        # assigning a random color to each cell but not to the cells at the edges which are above a size of 10,000 
        if sizes[i] <=10000 : #MAX CELL SIZE
            #e.g. if i is 1, the output is 2 means all the pixels labelled 2 i.e. 1 cell will be assigned 1 random colour
#[0] is for blue, [1] is for red and [2] is for green defined under colour[] later.
            b1[output == i + 1]=colors[i][0]
...
def process_image2(img,a,b,c,d):
    if c <1:
        c=1
    #Note: 
 #cv.GaussianBlur blurs the image and gives the first image in GUI sequence.  
...
#The output of this is the second image in the GUI sequence of images being displayed. 
  ...
  #This function removes small blobs and gives the 3rd image in the sequence of images in GUI. 
    ...
    ret,img6 = cv.threshold(img5,b,255,cv.THRESH_BINARY)
    Skeletonized_Image = (skeletonize(img6//255) * 255).astype(np.uint8)
    Watershed=watershed2(Skeletonized_Image)[0]  
    img6=cv.cvtColor(img6.astype(np.uint8), cv.COLOR_GRAY2BGR)
    #this gives a red skeleton to the membrane which is seen in image 5 in GUI
    Skeletonized_Image_BGR=merges_red(img//2,Skeletonized_Image,255)
 #Next, the skeletonized image is converted to a watershed and this is image 6 in the sequence of images. 
...
#Basepath is the path to the stacks folder with the images.
photos=[]
...
#if you plot list_of_means you find distinct peaks in intensity of each image studied 
...
Info_str="""
 ...
 User Interface:   
    ...
    If you have any questions or comments please email dmg8@st-andrews.ac.uk
...
#image refers to the two best images i.e. ones with highest resolution/definition;
...
#creating a loop until break is used
    while(1):
        ...
        if k == ord('b'):
        #if clicked on b, increase the diameter of the circle by 3 pixels 
            dim+=3
            
        if k == ord('n'):
        #if clicked on n, decrease the diameter of the circle by 3 pixels 
            #need to check if dim is >3 to prevent negative diameter
            if dim>=3:
                dim-=3
            else:
                dim=1
...
#This is the manual mode with two images 
        if Display_Mode==False:
...
#This is the mode where you can only change threholds but not manual editing
        if Display_Mode==True: 
...
#loop is done
    cv.destroyAllWindows()
#create the watershed image, and save it in outcome
    watershed_Image=watershed(saved_list[-1])[0]
...
#image refers to the two best images i.e. ones with highest resolution/definition;
#image [0] is the one with highest local maxima and image[1] is the one before image [0]
while 0 < iter_photo < len(tiff_images):
    Cursor=np.zeros((dim1,dim2)).astype(np.uint8)
    Numbers=np.zeros((dim1,dim2)).astype(np.uint8)
    Image=tiff_images[iter_photo]
    Image_str='Image'+str(iter_photo+1) 
...
#creating a loop until break is used
    ...
    #s is to save all the work 
        if k==ord('s'):
            watershed_Image=watershed2(saved_list[-1])[0]
        #every time you make a modification it will not be saved, Only the last element in saved list will be saved to saved_list[-1]
            outcome[iter_photo]=(list((saved_list[-1],watershed_Image,saved_list[-1],update_numbers(saved_list[-1],iter_photo))))
...
#This is the manual mode with two images 
        if Display_Mode==False:
#take saved_list[-1] is watershed i.e. there is a color to every cell
            Watershed=watershed2(saved_list[-1])
#The right panel will have the watershed with all 3 channels denoted by Watershed[0] 
            right_panel=Watershed[0]
#on the left panel we have the two images with the highest resolution which was initially multipled by 2 so here we divide by 2 again
#make all the membranes red where a1*255 is on the other end of the contours switch on trackbar
...
#These are the trackbars for the automatic popup
            ...
            d2 = cv.getTrackbarPos('On/Off',Image_str) #no particular fucntion
#calls the function process_image defined earlier to process the images in this popup         
            frames=process_image2(Base_Image,a2,b2,c2,d2)
#this horizontally stacks 3 images one beside each other in the first row of images
            horizontal1 = np.hstack((frames[0],frames[1],frames[2]))
            bgrhorizontal1 = cv.cvtColor(horizontal1.astype(np.uint8), cv.COLOR_GRAY2BGR)
#this horizontally stacks 3 images one beside each other in the second row of images
