...
import fnmatch #fnmatch is a module for filename pattern matching
...
def GUI(event,x,y,flags,param):
    ...
    #each time you modify image you want to save the changes and update the cell numbers each time
                ...
                drawing=False 
                been=False
   #if you click once don't draw a line but if you click the seond time draw a line betweem the first click and the second            
    if been ==True:
        if drawing==False:
            if mode==True:
                if event == cv.EVENT_LBUTTONDOWN:
...
#               saving_location ='saved_work'
    ...
    if frame_num > 1:
        CELL_DICTIONARY=save_excel(outcome,saving_location + '_cell_info.xlsx')
...
photos.sort()
#print('number of photos is ', len(photos))

#find the size of each stack and the number of stacks and store them as variables
zinstack=int(len(fnmatch.filter(photos, '*t001*' )))   #the numer of images that have 't001' in their filename
stackno=int(len(photos)/zinstack)  #calculating the number of stack using the variable above 
halfstack=int(np.round(zinstack/2)) #might need to change it to a third, ask Marcus
...
#ask for the range to look at
r_answer=gui.buttonbox('Where would you like to find the brightest image?', choices=('Top half of the stack', 'Full stack'))

#the value of the variable z_stop (which will be the end of the range under investigation) depends on the answer 
if r_answer=='Top half of the stack':
    z_stop=halfstack
if r_answer=='Full stack':
    z_stop=zinstack
...
brightnesses=[]
def solver():
    #separate the stacks from one another (pick one at a time), and find the brightest image
    #create a list for storing the brightest images and their brightness
    for t in range(stackno):
        #print('t is now: ', t)
        if t==0:
            #introduce a temporary variable for the starting point of the stack
            stackstart=t*zinstack
            #create a temporary list to store the stack
            temp_list_of_means=[]
            #add all the images in a stack to the list
            for z in range(stackstart, stackstart+z_stop, 1):
                temp_list_of_means.append(list_of_means[z])
            #find the brightest image in the stack
            max_brightness=np.max(temp_list_of_means)
            #store the index of the brightest image in the stack in the list of local maxima
            maxima_index=list_of_means.index(max_brightness)
            local_maxima_list.append(maxima_index)
            brightnesses.append(max_brightness)
        else:
            stackstart=int(t*zinstack)
            #define the start and endpoints of the range to investigate
            rangestart=int(stackstart+maxima_index-2)
            rangestart=np.maximum(rangestart,stackstart)
            rangeend=int(stackstart+maxima_index+2)
            rangeend=np.minimum(rangeend,stackstart+zinstack)
            #investigate the range like in the first case
            temp_list_of_means=[]
            for z in range(rangestart, rangeend, 1):
                temp_list_of_means.append(list_of_means[z])
            max_brightness=np.max(temp_list_of_means)
            maxima_index=list_of_means.index(max_brightness)
            #add the maxima index to the list which will carry it forward to the next section of code
            local_maxima_list.append(maxima_index)
            brightnesses.append(max_brightness)
            #then reassign maxima index so the next iteration can correctly handle it
            maxima_index=maxima_index-stackstart
    return brightnesses,local_maxima_list

#run the solvar to find the brightest image in the specified part of the stack
solver()    


#create the string to display in the button box 
lvl_str=str(local_maxima_list[0])
darkest_maxima=str(np.min(brightnesses))
text=str('The selector found z='+lvl_str+' as a baseline for analysis. The minimum brightness in the level is '+darkest_maxima)
#create the pop-up asking for approval
check=gui.buttonbox(text, choices=('Proceed', 'Find a brighter level in the full stack'))

if check=='Find a brighter level in the full stack':
    local_maxima_list=[]
    z_stop=zinstack
    solver()
...
#creating a loop until break is used
    while(1):
        ...
        message = 'Controls' 
        optionbox = gui.buttonbox(message, choices=('Switch Drawing Mode', 'Next Frame', 'Previous Frame', 'Eraser diam +', 'Eraser diam -', 'Undo', 'Change Segmentation Mode', 'Save'))
            
        if optionbox == 'Switch Drawing Mode':
            mode = not mode
                
        if optionbox == 'Next Frame':
            lag_iter=iter_photo
            iter_photo+=1
            break
            
        if optionbox == 'Previous Frame':
            if iter_photo>0:
                lag_iter=iter_photo
                iter_photo-=1
                break
                    
        if optionbox == 'Eraser diam +':
            dim+=5
        ...
        if optionbox == 'Change Segmentation Mode':
